...
_filtro_keys = ['filtro_rep', 'filtro_cor', 'filtro_seg', 'filtro_ramo', 'filtro_util', 'filtro_tp_emissao', 'filtro_regiao', 'filtro_uf', 'filtro_apolice', 'slider_anos']

if st.sidebar.button('Limpar Todos os Filtros'):
    for k in _filtro_keys:
        if k in st.session_state:
            # slider usa tupla, multiselects usam lista
            if k == 'slider_anos':
                del st.session_state[k]   # deletar força o slider a voltar ao valor padrão
            else:
                st.session_state[k] = []
    st.rerun()
